...
function FastBootConfig(inputNode, options) {
  ...
  // what if app does not set generateAssetMap option?
  var defaultAssetMapPath = 'assets/assetMap.json';
  var assetRev = this.project.addons.filter(function(addon) {
    // what happens when broccoli-asset-rev is not included explicitly? ember-cli does not assume
    // consuming app will have this.
...
FastBootConfig.prototype.buildManifest = function() {
  var appFile = this.outputPaths.app.js.split('/').splice(1).join('/');
  var appFastbootFile = this.outputPaths.fastboot.app.split('/').splice(1).join('/');
  var vendorFile = this.outputPaths.vendor.js.split('/').splice(1).join('/');
  ...
  if (rewrittenAssets) {
    ...
    for (var key in rewrittenAssets.assets) {
      assets[key] = rewrittenAssets.assets[key];
    }
    ...
    manifest['vendorFile'] = assets[manifest['vendorFile']];
  }
