...
module.exports = {
  ...
  runCommand: function(appName, options) {
    ...
    var pkg = fs.readFileSync(path.join(outputPath, 'package.json'));
    pkg = JSON.parse(pkg);

    manifest = pkg.fastboot.manifest;

    var server = new FastBootServer({
      appFile: path.join(outputPath, manifest.appFile),
      vendorFile: path.join(outputPath, manifest.vendorFile),
      htmlFile: path.join(outputPath, manifest.htmlFile),
...
function findConfigFile(outputPath) {
  return findFile('config', path.join(outputPath, 'fastboot-config.json'));
}
