  ...
  describe('chunks()', function () {
    ...
    describe('when there is a shoebox', function () {
      beforeEach(function () {
        ...
        doc.body.appendChild(
          doc.createRawHTMLSection(
            '<script type="fastboot/shoebox" id="shoebox-something">{ "some": "data" }</script>'
          )
        );
      ...
      it('returns a chunks for the head, body and shoebox', function () {
        return result.chunks().then(function (result) {
          ...
          expect(result[2]).to.eq(
            '<script type="fastboot/shoebox" id="shoebox-something">{ "some": "data" }</script><script type="x/boundary" id="fastboot-body-end"></script></body></html>'
          );
        });
      });
    });
    ...
    describe('when there are multiple shoeboxes', function () {
      beforeEach(function () {
        ...
        doc.body.appendChild(
          doc.createRawHTMLSection(
            '<script type="fastboot/shoebox" id="shoebox-something-a">{ "some": "data" }</script>'
          )
        );
        doc.body.appendChild(
          doc.createRawHTMLSection(
            '<script type="fastboot/shoebox" id="shoebox-something-b">{ "some": "data" }</script>'
          )
        );
        doc.body.appendChild(
          doc.createRawHTMLSection(
            '<script type="fastboot/shoebox" id="shoebox-something-c">{ "some": "data" }</script>'
          )
        );
      ...
      it('returns a chunks for the head, body and shoebox', function () {
        return result.chunks().then(function (result) {
          ...
          expect(result[4]).to.eq(
            '<script type="fastboot/shoebox" id="shoebox-something-c">{ "some": "data" }</script><script type="x/boundary" id="fastboot-body-end"></script></body></html>'
          );
        });
      });
    });
